 ...
 * Emits updates periodically to allow for real-time relative time updates in the UI.
 */
@Singleton
class NotificationRepositoryImpl @Inject constructor() : NotificationRepository {
     ...
     * Emits the same list every 60 seconds to trigger UI refreshes for relative timestamps.
     * 
    ...
    override fun getNotifications(): Flow<List<Notification>> = flow {
        while (true) {
            ...
            // Refresh every minute to update relative time ("5 min ago" -> "6 min ago")
            delay(60000)
        }
    }
}
